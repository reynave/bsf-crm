    ...
    function searchCustomer()
    {
        ...
        if ($status) {
            ...
            if(model("Core")->x_employee_type() == ''  ){
                ...
                if(isset($post['aks']) && $post['aks'] == 'true') {
                $where = " WHERE active = true AND customer_rank > 0 AND x_aks = true AND name like '%" . $name . "%'  ";
                }
                if(isset($post['aks']) && $post['aks'] != 'true'){
                $salesId = $post['salesId'];
                $where = " WHERE active = true AND customer_rank > 0 AND (x_aks = false OR x_aks IS NULL)";
                $where .= " AND x_kodeareaid IN (SELECT id FROM x_masterwilayah WHERE x_salesperson = '".$salesId."')";
            ...
            if (isset($post['field']) && $post['field'] == 'contact') {
                $salesId = $post['salesId'];
                $where = " WHERE x_kodeareaid IN (SELECT id FROM x_masterwilayah WHERE x_salesperson = '".$salesId."')";
                $where .= " AND name like '%" . $name . "%' AND   x_end_user = true  ";
            }
